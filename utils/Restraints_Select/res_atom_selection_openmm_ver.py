...
class Res_atom_select():
    ...
    strategy_list = ['lig_shape', 'HB_pair', 'HB_mainchain', 'Huggins'], fix_lig_3atoms_index = []
                                ):
        ...
        strategy_list: list
            A list of strategies to select restraint atom groups. Default: ['lig_shape', 'HB_pair', 'HB_mainchain', 'Huggins', 'Fix_lig_3atoms'].
        fix_lig_3atoms_index: list
            A list containing the index of the three atoms from ligand needed for restraint.(Start from 1) Default: [].
        ...
        if 'HB_pair' in strategy_list:
            ...
            lst_a = []
        # Third strategy
        if 'HB_mainchain' in strategy_list:
            ...
            lst_b = []
        #Forth strategy
        if 'Huggins' in strategy_list:
            d = Sel_rest_atm(self.traj, self.parm, self.top_file, self.xyz_file)
            lst_d = d.main()
            self.based_Huggins = lst_d
            if iflog:
                grp_num4=len(lst_d)
                print(f"Selected by forth_stategy: {lst_d} with number of {grp_num4}")
                lst_d_file = open('based_Huggins', 'w+')
                print(lst_d, file=lst_d_file)
                lst_d_file.close()
        else:
            lst_d = []

        if 'Fix_lig_3atoms' in strategy_list:
            assert len(fix_lig_3atoms_index) == 3, "Error: fix_lig_3atoms_index should be a list containing the index of the three atoms from ligand needed for restraint.(Start from 1)"
            lst_e = fix_ligand_3atoms_based_sel(self.traj, fix_lig_3atoms_index, lig_resi_name)
            if iflog:
                grp_num5=len(lst_e)
                print(f"Selected by fifth_stategy: {lst_e} with number of {grp_num5}")
                lst_e_file = open('based_Fix_lig_3atoms', 'w+')
                print(str(lst_e), file=lst_e_file)
                lst_e_file.close()
        else:
            lst_e = []
        ...
        if len(strategy_list) == 0:
            print(f"Error: There's not a strategy in the strategy_list")
        ...
        if iflog:
            ...
            print(f"After deduplication, Selected by all_stategy: {six_atoms} with number of {grp_num4}")
