...
class RestraintState2(GlobalParameterState):
    """
    ...
    """
    lambda_restraints2 = GlobalParameterState.GlobalParameter('lambda_restraints2', standard_value=1.0)

    @lambda_restraints2.validator
    def lambda_restraints2(self, instance, new_value):
        if new_value is None:
            return None
        if not (0.0 <= new_value <= 1.0):
            raise ValueError('lambda_restraints2 must be between 0.0 and 1.0')
    ...
    def __setstate__(self, serialization):
        try:
            ...
            lambda_restraints2 = serialization['lambda_restraints2']
            self._initialize(parameters_name_suffix=None, lambda_restraints2=lambda_restraints2)
...
class BoreschLike(ReceptorLigandRestraint, ABC):
    ...
    *Warning*: Only heavy atoms can be restrained. Hydrogens will automatically be excluded.
    ...
    def restrain_state(self, thermodynamic_state):
        ...
        restraint_force.addGlobalParameter('lambda_restraints2', 1.0)
...
class PeriodicTorsionBoresch(Boresch2):
    """
